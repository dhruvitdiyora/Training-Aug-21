...
SELECT CONCAT(FirstName, ' ', LastName) AS 'Name' FROM Employees WHERE ManagerID IN (SELECT D.ManagerID FROM Departments D JOIN Locations L ON L.LocationID=D.LocationID WHERE L.CountryID='US' AND D.ManagerID!=0)

--task4. Find the names (first_name, last_name) of the employees who are managers. 
SELECT CONCAT(FirstName, ' ', LastName) AS 'Name' FROM Employees WHERE EmployeeID IN (SELECT ManagerID FROM Employees)

--task5. Find the names (first_name, last_name), salary of the employees whose salary is greater than the average salary. 
SELECT CONCAT(FirstName, ' ', LastName) AS 'Name',Salary FROM Employees WHERE Salary > (SELECT AVG(Salary) FROM Employees)
...
SELECT CONCAT(FirstName, ' ', LastName) AS 'Name',Salary FROM Employees WHERE Salary > (SELECT AVG(Salary) FROM Employees) AND DepartmentID=(SELECT DepartmentID FROM Departments WHERE DepartmentName='IT')

--task8. Find the names (first_name, last_name), salary of the employees who earn more than Mr. Bell. 
SELECT CONCAT(FirstName, ' ', LastName) AS 'Name',Salary FROM Employees WHERE Salary > (SELECT Salary FROM Employees WHERE LastName='Bell')
...
SELECT E.EmployeeID, E.FirstName, E.LastName, E.Salary FROM Employees E JOIN (SELECT AVG (Salary) AS Average_Salary, DepartmentID FROM Employees GROUP BY DepartmentID) temp ON E.DepartmentID = temp.DepartmentID WHERE E.Salary > temp.Average_Salary
